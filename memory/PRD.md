...
- **Iter 86 · 2026-02**: GSAP Phase 2 Revert (Mert) — User-Wunsch: "rückgängig auf zuletzt Health State, Fokus auf Deploy + Stripe". (1) `MagneticCursor` Komponente komplett entfernt (File gelöscht + Import/Usage aus `App.js` raus). Native Cursor zurück. (2) `PageTransition` Komponente entfernt (File gelöscht, ungenutzter Import aus `App.js` raus). (3) `DashboardPage.js`: `useMotion` staggered-reveal Animation komplett entfernt (war Ursache der "Dashboard-Probleme" — versteckte Elemente bei FX-Hang). `rootRef` + `useRef`-Import + `data-anim`-Animation-Trigger entfernt. (4) Behalten: `useMotion` Hook selbst (noch genutzt in `LoginBrandPanel`, `MyPathPage`, `LearningVideosTab` — keine User-Beschwerde), `VoiceWaveVisualizer` (legitimes Feature in `VoicePlayButton`). Build ✅, Smoke-Screenshot ✅ (Cursor=auto, keine `.magnetic-cursor-dot` mehr im DOM). **NÄCHSTER STEP**: Stripe Live Activation — User schickt `sk_live_...` + `whsec_...` Webhook-Secret, Webhook konfiguriert auf `https://leader-os.de/api/webhook/stripe`.
