...
- **Iter 83 · 2026-02 (Deployment Hotfix)**: Production-Backend crashte mit `ModuleNotFoundError: No module named 'sentry_sdk'`. Root Cause: `posthog` Import in `routes/monitoring.py` war komplett aus `requirements.txt` vergessen, und `sentry-sdk` Build-Layer war stale. Fix: `pip install 'sentry-sdk[fastapi]==2.18.0' 'posthog==3.7.0'`, `requirements.txt` ergänzt um `posthog==3.7.0`, `backoff==2.2.1`, `monotonic==1.6` (transitive deps von posthog). Verifiziert: `/api/health` → `{"status":"ok"}`, `/api/monitoring/health` → `{"sentry":true,"posthog":false,"env":"preview"}`. Backend bootet sauber ohne Stacktrace. Ready für Re-Deployment.
