...
export const WladHubEmptyState = ({ de, onSync, syncing }) => (
  <Card className="border-black/[0.04] dark:border-white/[0.06] overflow-hidden animate-fade-in" data-testid="wladhub-card-empty">
    <div className="bg-gradient-to-r from-[#0A0A0A] to-[#111] p-4 text-foreground">
      <div className="flex items-center gap-2 mb-2">
        <div className="w-7 h-7 rounded-lg bg-[#BFFF00]/15 flex items-center justify-center">
          <Zap size={14} className="text-brand" />
        </div>
        <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-muted-foreground">Leader-Diagnose</span>
      </div>
      ...
      <div className="flex gap-2">
        ...
        <Button size="sm" variant="outline" onClick={onSync} disabled={syncing}
          className="border-border text-foreground hover:bg-foreground/10 text-[10px] h-7" data-testid="wladhub-sync-btn">
