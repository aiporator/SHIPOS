    ...
    <div className="fixed bottom-6 right-6 z-50 w-[360px] max-w-[calc(100vw-2rem)] wlad-slide-up" data-testid="bot-mascot-panel">
      <div className="bg-white dark:bg-card rounded-2xl shadow-2xl shadow-black/15 border border-black/[0.06] dark:border-white/[0.06] overflow-hidden">
        ...
        <div className="px-4 pb-3 space-y-2">
          <ActionButton icon={MessageSquareText} iconBg="bg-sky-50 dark:bg-sky-500/10 dark:bg-sky-50 dark:bg-sky-500/100/10" iconColor="text-sky-600" label={de ? 'KI-Coach starten' : 'Start AI Coach'} sub={de ? '5 spezialisierte Rollen' : '5 specialized roles'} testId="wlad-action-chat" onClick={() => onNavigate('/chat')} />
          ...
          <ActionButton icon={Calendar} iconBg="bg-purple-50 dark:bg-[#7B3FE4]/10" iconColor="text-purple-600" label={de ? 'Coaching-Call buchen' : 'Book Coaching Call'} sub={de ? '1:1 mit Wlad' : '1:1 with Wlad'} testId="wlad-action-book" onClick={() => onNavigate('/coaching')} />
        </div>
